function backupFiles($profile_path) {
    mkdir $($profile_path + "backup") -ErrorAction Stop
    Copy-Item $($profile_path + "atvantage.properties*") $($profile_path + "\backup") -ErrorAction Stop
}

function renameFiles($profile_path, $old_env, $new_env) {
    backupFiles $profile_path
    backupFiles $($profile_path + $bin)
    Rename-Item -Path $($profile_path + $atvantage) -NewName $($atvantage + "." + $old_env) -ErrorAction Stop
    Rename-Item -Path $($profile_path + $bin + $atvantage) -NewName $($atvantage + "." + $old_env) -ErrorAction Stop
    Rename-Item -Path $($profile_path + $atvantage + "." + $new_env) -NewName $atvantage -ErrorAction Stop
    Rename-Item -Path $($profile_path + $bin + $atvantage + "." + $new_env) -NewName $atvantage -ErrorAction Stop
    Remove-Item $($profile_path + "backup") -recurse
    Remove-Item $($profile_path + $bin + "backup") -recurse 
}

function rollback($profile_path) {
    Copy-Item $($profile_path + "\backup\atvantage.properties*") $profile_path -ErrorAction Stop
    Remove-Item $($profile_path + "\backup") -recurse -ErrorAction Stop
}
